...
* **Blocking I/O:** Ứng dụng gọi hàm $\rightarrow$ Thread bị đóng băng, OS gỡ Thread ra khỏi CPU $
ightarrow$ Chờ dữ liệu xong $
ightarrow$ Thread chạy tiếp.
* **Non-blocking I/O:** Ứng dụng gọi hàm $\rightarrow$ Trả về kết quả ngay (thành công hoặc báo "chưa xong") $
ightarrow$ Thread rảnh tay làm việc khác $
ightarrow$ Xử lý dữ liệu sau khi nhận được tín hiệu sẵn sàng từ OS.
